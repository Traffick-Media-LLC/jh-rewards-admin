...
const AdminApp: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const [usersRes, ordersRes, productsRes, pointsRes] = await Promise.all([supabase.from("profiles").select("id").eq("marketing_emails", true), supabase.from("orders").select("id, total_points, status"), supabase.from("products").select("id, active"), supabase.from("points_transactions").select("points, type")]);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-6">
          ...
          <TabsContent value="products" className="space-y-6">
            ...
            <ProductFilters searchTerm={productSearchTerm} onSearchChange={setProductSearchTerm} categoryFilter={productCategoryFilter} onCategoryChange={setProductCategoryFilter} statusFilter={productStatusFilter} onStatusChange={setProductStatusFilter} sortBy={productSortBy} onSortChange={setProductSortBy} onClearFilters={() => {
            ...
            {selectedProducts.length > 0 && <BulkProductActions products={products || []} selectedProducts={selectedProducts} onSelectionChange={setSelectedProducts} />}
            ...
            <Card>
              ...
              <CardContent>
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-12">
                        <input type="checkbox" checked={products?.length > 0 && selectedProducts.length === products.length} onChange={e => {
                        if (e.target.checked) {
                    ...
                    {products?.map(product => <TableRow key={product.id}>
                        ...
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-3">
                            {product.image_url ? <img src={product.image_url} alt={product.name} className="w-10 h-10 object-cover rounded border" /> : <div className="w-10 h-10 bg-muted rounded border flex items-center justify-center">
                                <span className="text-xs text-muted-foreground">No Image</span>
                              </div>}
                            <div>
                              {product.name}
                              {product.homepage && <Badge variant="outline" className="ml-2 text-xs">
                                  Featured
                                </Badge>}
                            </div>
                          </div>
                        ...
                        <TableCell>
                          <div>
                            ...
                            {product.sale_price_cents && <div className="text-xs text-muted-foreground line-through">
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            ...
                            <Switch checked={product.active} onCheckedChange={checked => toggleProductStatus(product.id, checked)} />
                          </div>
                        </TableCell>
                      </TableRow>)}
                  </TableBody>
                ...
                {products?.length === 0 && <div className="text-center py-8 text-muted-foreground">
                    {productSearchTerm || productCategoryFilter || productStatusFilter ? "No products match your filters" : "No products found. Create your first product to get started."}
                  </div>}
              </CardContent>
            </Card>
          </TabsContent>
